...
# from datasheet for IMX219PQH5-C, see file:///C:/Users/lisa/Desktop/6.%20semester/Sensorer%20og%20Instrumetering/Lab/Lab3/kamera%20datablad.pdf
red_wavelength = 600
...
# Optisk penetrasjonsdybde delta [m]
delta = np.sqrt(1.0 / (3.0 * (musr + mua) * mua))

print("Resultater:")
for name, wl, mua_i, musr_i, delta_i in zip(channel_names, wavelength, mua, musr, delta):
    print(
        f"{name:>5}: "
        f"lambda = {wl:6.1f} nm, "
        f"mua = {mua_i:10.3f} 1/m, "
        f"musr = {musr_i:10.3f} 1/m, "
        f"delta = {delta_i*1e3:7.3f} mm"
    )
